...
$reportquery=$focus1->sGetSQLforReport($reportid,$nu);
   ...
   $query=$quer[0]."  ";
$i1=0;
		$rspotsmax=$adb->query("SELECT vtiger_project.raggruppamento_giorni as gr, REPLACE(CONCAT('\"',vtiger_project.raggruppamento_giorni,'\"'),\"'\",\"\") as grpc FROM $query and  vtiger_project.raggruppamento_giorni<> '' and vtiger_project.raggruppamento_giorni is not null group by vtiger_project.raggruppamento_giorni");
...
$dt=Array();
       ...
       $rspots1=$adb->query("SELECT    FROM $query  and  REPLACE(CONCAT(vtiger_project.linktobuyer),\"'\",\"\")='$ptc' and vtiger_project.raggruppamento_giorni=\"$cat[$m]\" ");
       $mainfld1=explode(",",""); 
       $fagg1=explode(",","");
           if(""=="1"){
            $dttod=date("Y-m-d",strtotime("-1 years",strtotime(date("Y-m-d"))));
             $dttod2=date("Y-m-d",strtotime("-2 years",strtotime(date("Y-m-d"))));
       $rspots22=$adb->query("SELECT    FROM $query  and  REPLACE(CONCAT(vtiger_project.linktobuyer),\"'\",\"\")='$ptc' and DATE_FORMAT(vtiger_project.customer_purchase_data,'%Y-%m-%d')>'$dttod2' and DATE_FORMAT(vtiger_project.customer_purchase_data,'%Y-%m-%d')<='$dttod' and vtiger_project.raggruppamento_giorni=\"$cat[$m]\"");
             
}
  $cat1=str_replace(array(".",",","_"," "),array("","","",""),strtolower($cat[$m]));
          $xf=str_replace(array(".",",","_"," "),array("","","",""),strtolower("Giorni apertura"));
...
$ag2[$ii]+=number_format($adb->query_result($rspots1,0,"$mf"),0,",",".");
     ...
     if(""=="1"){
            $dttod=date("Y-m-d",strtotime("-1 years",strtotime(date("Y-m-d"))));
             $dttod2=date("Y-m-d",strtotime("-2 years",strtotime(date("Y-m-d"))));
       $rspots2=$adb->query("SELECT    FROM $query  and  REPLACE(CONCAT(vtiger_project.linktobuyer),\"'\",\"\")='$ptc' and DATE_FORMAT(vtiger_project.customer_purchase_data,'%Y-%m-%d')>'$dttod2' and DATE_FORMAT(vtiger_project.customer_purchase_data,'%Y-%m-%d')<='$dttod'");
...
$frm[$ik]+=$adb->query_result($rspots3,0,"form");}
  ...
  if(""!=""){
      ...
      $fcols[$ik]+=$f1[$ik];
        ...
        for($m=0;$m<sizeof($cat);$m++){
       $rspots1=$adb->query("SELECT    FROM $query  and vtiger_project.raggruppamento_giorni=\"$cat[$m]\" ");
       $mainfld1=explode(",",""); 
       $fagg1=explode(",","");
           $cat1=str_replace(array(".",",","_"," "),array("","","",""),strtolower($cat[$m]));
          $xf=str_replace(array(".",",","_"," "),array("","","",""),strtolower("Giorni apertura"));
