...
global $adb;
$fp = fopen('fsdfsdfssd_customada1093c82fe730322aeda7c15167735.csv', 'a');
...
$checkf=explode(",","1,,1,,,,,,,,1");
 ...
 $group=array();
  $k=0;
  $fld=explode(",","ProductsID,ProductsProductName,ProductsBrandprodotto,ProductsDescrizioneAggiuntiva,Productslinktoproductscategory,ProductsGuastoarendere,ProductsTeknemaUnitPrice,ProductsCATUnitPrice,ProductsCustomerUnitPrice,ProductsAttivo");
...
$qr1=explode("order by",$rq[1]);
        ...
        $qr=strtolower($rq[0]." ,.id as primid FROM ".$rq[1]);}
              ...
              $query1="show columns from mv_246Prodottitpv1246Report";
...
$j=0;
	...
	for($i1=1;$i1<=$num_rows1;$i1++)
	{
       ...
       $md1=$adb->query_result($tab,0,"name");
             ...
             $fl=implode(",",$fl);
                $rq=explode("from","select $fl from mv_246Prodottitpv1246Report where 1=1 ");
...
$group2=implode(",",$group);
$qr1=explode("order by",$rq[1]);
                $qr=strtolower("select $fields 1 as primid FROM ".$qr1[0]." group by $group2");
}
  else{
