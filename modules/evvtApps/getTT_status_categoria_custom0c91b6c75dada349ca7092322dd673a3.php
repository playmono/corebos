<?php
    ...
    $tit="(Categoria per Numero HelpDesk_Title)";
...
$dt=Array();
       ...
       $rspots1=$adb->query("SELECT  count(vtiger_troubletickets.title)  as vtigertroubleticketstitle  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.reportingcategory),\"'\",\"\")='$ptc' and vtiger_troubletickets.status=\"$cat[$m]\" ");
...
$ag[$ii]=$fagg1[$ii].": ".$adb->query_result($rspots1,0,"$mf");
       ...
       $content[$j]["$xf$cat1"]=implode(" - ",$ag);
        ...
        for($m=0;$m<sizeof($cat);$m++){
       $rspots1=$adb->query("SELECT  count(vtiger_troubletickets.title)  as vtigertroubleticketstitle  FROM $query  and vtiger_troubletickets.status=\"$cat[$m]\" ");
       $mainfld1=explode(",","vtiger_troubletickets.title"); 
       $fagg1=explode(",","Numero HelpDesk_Title");
               for($ii=0;$ii<sizeof($mainfld1);$ii++){
$mf=str_replace(array(".",",","_"),array("","",""),strtolower($mainfld1[$ii]));
