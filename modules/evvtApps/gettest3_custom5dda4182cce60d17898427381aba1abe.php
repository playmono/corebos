...
$reportquery=$focus1->sGetSQLforReport($reportid,$nu);
 ...
 $group=array();
  $k=0;
  $fld=explode(",","Accounts_ID,Accounts_Account_Name,Accounts_Ragione_Sociale,Accounts_Type,Accounts_Natura_Giuridica,Accounts_Partita_Iva,Accounts_Is_vendor,Accounts_Brand_id,Accounts_Codice_Fiscale,Accounts_Categoria_Contabile,Accounts_Assigned_To,Accounts_Incoterms,Accounts_IndustrySectorLevel,Accounts_IndustrySector,Accounts_Tipo_Fornitore,Accounts_PayamentsType,Accounts_DeliverysType,Accounts_GroupCompanies,Accounts_Password_prestashop,Accounts_Totale_Fatturato_Anno,Accounts_Totale_Ordinato_Anno,Accounts_Numero_fatture,Accounts_Numero_Ordini,Accounts_Numero_Insoluti,Accounts_Totale_Insoluto_Alla_data,Accounts_Data_ultima_fattura,Accounts_Data_ultimo_Ordine,Accounts_Employees,Accounts_Annual_Revenue,Accounts_Rating,Accounts_Range_di_fatturato,Accounts_Other_Email");
...
$group2=implode(",",$group);
                $qr=strtolower("select $fields vtiger_account.accountid as primid FROM ".$rq[1]." group by $group2");
}
        else{
        $qr=strtolower($rq[0]." ,vtiger_account.accountid as primid FROM ".$rq[1]);}
            }
...
$fld=explode(",","Accounts_ID,Accounts_Account_Name,Accounts_Ragione_Sociale,Accounts_Type,Accounts_Natura_Giuridica,Accounts_Partita_Iva,Accounts_Is_vendor,Accounts_Brand_id,Accounts_Codice_Fiscale,Accounts_Categoria_Contabile,Accounts_Assigned_To,Accounts_Incoterms,Accounts_IndustrySectorLevel,Accounts_IndustrySector,Accounts_Tipo_Fornitore,Accounts_PayamentsType,Accounts_DeliverysType,Accounts_GroupCompanies,Accounts_Password_prestashop,Accounts_Totale_Fatturato_Anno,Accounts_Totale_Ordinato_Anno,Accounts_Numero_fatture,Accounts_Numero_Ordini,Accounts_Numero_Insoluti,Accounts_Totale_Insoluto_Alla_data,Accounts_Data_ultima_fattura,Accounts_Data_ultimo_Ordine,Accounts_Employees,Accounts_Annual_Revenue,Accounts_Rating,Accounts_Range_di_fatturato,Accounts_Other_Email");
    ...
    if($count!=0){
for($i=0;$i<$count;$i++){
   $content[$i]["accountid"]=$adb->query_result($query,$i,"primid");
...
$us=getUsername($adb->query_result($query,$i,"$name"));
   ...
   $content[$i]["$name1"]=$us;}
       ...
       $content[$i]["url"]="index.php?module=$md1&action=DetailView&record=". $content[$i]["accountid"];
       }
