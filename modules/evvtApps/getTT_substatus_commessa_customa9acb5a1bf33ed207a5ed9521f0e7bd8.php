<?php
    ...
    $tit="(Status per Numero HelpDesk_Title)";
...
$dt=Array();
		$rspots=$adb->query("SELECT REPLACE(CONCAT(vtiger_troubletickets.status),\"'\",\"\") as cols FROM $query GROUP BY vtiger_troubletickets.status");
		$data=Array();
                $j=0;
        ...
        $ptc=$pt['cols'];
                       ...
                       $content[$j]["vtigertroubleticketsstatus"]=$ptc;
                      
        for($m=0;$m<sizeof($cat);$m++){
       $rspots1=$adb->query("SELECT  count(vtiger_troubletickets.title)  as vtigertroubleticketstitle  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.status),\"'\",\"\")='$ptc' and vtiger_troubletickets.reportingcategory=\"$cat[$m]\" ");
       ...
       $content[$j]["$xf$cat1"]=implode(" - ",$ag);
         $content[$j]["totale"]=implode(" - ",$ag1);
			...
			$j++;
                ...
                $content[$j]["ticketid"]=$j;
          $content[$j]["vtigertroubleticketsstatus"]="Totale";
