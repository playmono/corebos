<?php
    ...
    $tit="(DATA per Numero Giorni_Anno)";
...
$dt=Array();
		$rspots=$adb->query("SELECT REPLACE(CONCAT(vtiger_troubletickets.opendatatext),\"'\",\"\") as cols FROM $query GROUP BY vtiger_troubletickets.opendatatext");
		$data=Array();
                $j=0;
        ...
        $ptc=$pt['cols'];
                       ...
                       $content[$j]["vtigertroubleticketsopendatatext"]=$ptc;
              unset($ag1);        
        for($m=0;$m<sizeof($cat);$m++){
       $rspots1=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid,count(vtiger_troubletickets.giornianno)  as vtigertroubleticketsgiornianno  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.opendatatext),\"'\",\"\")='$ptc' and vtiger_troubletickets.sorgentett=\"$cat[$m]\" ");
       $mainfld1=explode(",","vtiger_crmentity.crmid,vtiger_troubletickets.giornianno"); 
       $fagg1=explode(",","Numero HelpDesk_ID,Numero Giorni_Anno");
           if(""=="1"){
            $dttod=date("Y-m-d",strtotime("-1 years",strtotime(date("Y-m-d"))));
             $dttod2=date("Y-m-d",strtotime("-2 years",strtotime(date("Y-m-d"))));
       $rspots22=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid,count(vtiger_troubletickets.giornianno)  as vtigertroubleticketsgiornianno  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.opendatatext),\"'\",\"\")='$ptc' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')>'$dttod2' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')<='$dttod' and vtiger_troubletickets.sorgentett=\"$cat[$m]\"");
             
}
  $cat1=str_replace(array(".",",","_"," "),array("","","",""),strtolower($cat[$m]));
          $xf=str_replace(array(".",",","_"," "),array("","","",""),strtolower("Sorgente_TT"));
...
$ag2[$ii]+=$adb->query_result($rspots1,0,"$mf");
     ...
     if(""=="1"){
            $dttod=date("Y-m-d",strtotime("-1 years",strtotime(date("Y-m-d"))));
             $dttod2=date("Y-m-d",strtotime("-2 years",strtotime(date("Y-m-d"))));
       $rspots2=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid,count(vtiger_troubletickets.giornianno)  as vtigertroubleticketsgiornianno  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.opendatatext),\"'\",\"\")='$ptc' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')>'$dttod2' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')<='$dttod'");
...
$agy2[$ii1]+=$adb->query_result($rspots2,0,"$mf");
  ...
  $rspots3=$adb->query("SELECT  sum($f[$ik]) as form  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.opendatatext),\"'\",\"\")='$ptc' ");
$content[$j]["formula$ik"]=$adb->query_result($rspots3,0,"form");
$frm[$ik]+=$adb->query_result($rspots3,0,"form");}
}
  if("1"!=""){
      $f1=explode("#",$content[$j]["aerroredeko"]+$content[$j]["aerrorehalilaj"]+$content[$j]["aerrorehoda"]+$content[$j]["aerroreosmeni"],$content[$j]["aareacompetenzanoncompleta/corretta"]+$content[$j]["aclientehasbagliatomandareavantilefreccie"]+$content[$j]["acliente/catrichiedonocambiamenti"]);
      ...
      $fcols[$ik]+=$f1[$ik];
			...
			$j++;
                ...
                $content[$j]["ticketid"]=$j;
          $content[$j]["vtigertroubleticketsopendatatext"]="Totale";
         $content[$j]["totale"]=implode(" - ",$ag2);  
        for($m=0;$m<sizeof($cat);$m++){
       $rspots1=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid,count(vtiger_troubletickets.giornianno)  as vtigertroubleticketsgiornianno  FROM $query  and vtiger_troubletickets.sorgentett=\"$cat[$m]\" ");
       $mainfld1=explode(",","vtiger_crmentity.crmid,vtiger_troubletickets.giornianno"); 
       $fagg1=explode(",","Numero HelpDesk_ID,Numero Giorni_Anno");
           $cat1=str_replace(array(".",",","_"," "),array("","","",""),strtolower($cat[$m]));
          $xf=str_replace(array(".",",","_"," "),array("","","",""),strtolower("Sorgente_TT"));
