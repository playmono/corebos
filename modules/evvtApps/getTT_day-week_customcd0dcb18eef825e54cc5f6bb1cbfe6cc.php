<?php
    ...
    $tit="(HelpDesk_Data_open per Numero HelpDesk_ID)";
...
$dt=Array();
       ...
       $rspots1=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.opendatatext),\"'\",\"\")='$ptc' and vtiger_troubletickets.reportingcategory=\"$cat[$m]\" ");
...
$agy2[$ii1]+=$adb->query_result($rspots2,0,"$mf");
  ...
  if(""!=""){
  $rspots3=$adb->query("SELECT  sum() as form  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.opendatatext),\"'\",\"\")='$ptc' ");
