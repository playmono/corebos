...
$dt=Array();
		$rspots=$adb->query("SELECT REPLACE(CONCAT(vtiger_crmentityHelpDesk.createdtime),\"'\",\"\") as cols FROM $query GROUP BY vtiger_crmentityHelpDesk.createdtime");
		$data=Array();
                $j=0;
       ...
       $rspots1=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid  FROM $query  and  REPLACE(CONCAT(vtiger_crmentityHelpDesk.createdtime),\"'\",\"\")='$ptc' and vtiger_troubletickets.status=\"$cat[$m]\" ");
