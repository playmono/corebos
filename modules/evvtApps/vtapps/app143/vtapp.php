<?php
    ...
    class Monitoraggio_Days_On_custom1a230bfb115cce0a0a087e487a278aaa extends vtApp {
...
$reportquery=$focus1->sGetSQLforReport($reportid,$nu);
 ...
 $group=array();
  $k=0;
  $fld=explode(",","Project_Project_Name,Project_LBLGROUPBUYER,Project_RMA,Project_Start_Date,Project_Substatus,Project_Stato_parte_chiamata,Project_Differenza");
...
global $adb,$current_language,$current_user;
		...
		$smarty->template_dir = $this->apppath;
                                  ...
                                  $fld=explode(",","Project_Project_Name,Project_LBLGROUPBUYER,Project_RMA,Project_Start_Date,Project_Substatus,Project_Stato_parte_chiamata,Project_Differenza,vis");
                                  $fldn=explode(",","Chiamata,Commitente,RMA,Data di inizio,Substatus,Stato parte,Diff.Days On,Visualizza");
                                          $fldnex=explode(","," ,Chiamata,Commitente,RMA,Data di inizio,Substatus,Stato parte,Diff.Days On,Visualizza");
                                        $colex=explode(","," ,Chiamata,Commitente,RMA,Data di inizio,Substatus,Stato parte,Diff.Days On ,Visualizza");
                                 ...
                                 $suba=explode(",",",,,,,,,");
                                        ...
                                        $wdth=explode(",","120,200,80,120,120,120,80,150");
                                     $f=Array();
                                     $s=Array();
                                     $a=Array();
                                    $g=Array();
 ...
 $l=0;
 $m=0;
 $fp = fopen('Monitoraggio_Days_On_custom1a230bfb115cce0a0a087e487a278aaa.csv', 'w');
    fputcsv($fp, $colex,";");
    fclose($fp);
  echo "<input type=\"button\" value=\"Export csv\" onclick=\"window.location.href='index.php?module=evvtApps&action=exportcsvMonitoraggio_Days_On_custom1a230bfb115cce0a0a087e487a278aaa'\">";
...
$smarty->assign("cond1",$condit);
                	return $smarty->display("ListViewKUIMonitoraggio_Days_On_custom1a230bfb115cce0a0a087e487a278aaa.tpl");
        }
