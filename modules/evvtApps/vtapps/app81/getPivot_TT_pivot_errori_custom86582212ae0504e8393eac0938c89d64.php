<?php
    ...
    $tit="(WEEK,GIORNO per Numero HelpDesk_ID)";
...
$dt=Array();
		$rspots=$adb->query("SELECT REPLACE(CONCAT(vtiger_troubletickets.fiscalwint,' ',vtiger_troubletickets.opendatatext),\"'\",\"\") as cols FROM $query GROUP BY vtiger_troubletickets.fiscalwint,vtiger_troubletickets.opendatatext");
		$data=Array();
                $j=0;
        ...
        $ptc=$pt['cols'];
                       ...
                       $content[$j]["vtigertroubleticketsfiscalwintvtigertroubleticketsopendatatext"]=$ptc;
              unset($ag1);        
        for($m=0;$m<sizeof($cat);$m++){
       $rspots1=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.fiscalwint,' ',vtiger_troubletickets.opendatatext),\"'\",\"\")='$ptc' and vtiger_troubletickets.status=\"$cat[$m]\" ");
       ...
       $rspots22=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.fiscalwint,' ',vtiger_troubletickets.opendatatext),\"'\",\"\")='$ptc' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')>'$dttod2' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')<='$dttod' and vtiger_troubletickets.status=\"$cat[$m]\"");
...
$ag2[$ii]+=$adb->query_result($rspots1,0,"$mf");
     ...
     if(""=="1"){
            $dttod=date("Y-m-d",strtotime("-1 years",strtotime(date("Y-m-d"))));
             $dttod2=date("Y-m-d",strtotime("-2 years",strtotime(date("Y-m-d"))));
       $rspots2=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.fiscalwint,' ',vtiger_troubletickets.opendatatext),\"'\",\"\")='$ptc' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')>'$dttod2' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')<='$dttod'");
...
$agy2[$ii1]+=$adb->query_result($rspots2,0,"$mf");
  ...
  $rspots3=$adb->query("SELECT  sum($f[$ik]) as form  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.fiscalwint,' ',vtiger_troubletickets.opendatatext),\"'\",\"\")='$ptc' ");
$content[$j]["formula$ik"]=$adb->query_result($rspots3,0,"form");
$frm[$ik]+=$adb->query_result($rspots3,0,"form");}
}
  if("1"!=""){
      $f1=explode("#",($content[$j]["ainprogress"]+$content[$j]["awaitforresponse"])/($content[$j]["ainprogress"]+$content[$j]["awaitforresponse"]+)$content[$j]["aclosed"]);
      ...
      $fcols[$ik]+=$f1[$ik];
			...
			$j++;
                ...
                $content[$j]["ticketid"]=$j;
          $content[$j]["vtigertroubleticketsfiscalwintvtigertroubleticketsopendatatext"]="Totale";
