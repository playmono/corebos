<?php
    ...
    class KPI_for_CAT_custom0fd4a0828ac7430cbff7cae2d1923f31 extends vtApp {
...
$reportquery=$focus1->sGetSQLforReport($reportid,$nu);
 ...
 $group=array();
  $k=0;
  $fld=explode(",","id,data,meseanno,commessa,committente,numerochiamate,check21,medialtp,checkltp,adjcheckltp,medialtr,checkltr,adjcheckltr,medialtrhr,checkltrhr,adjcheckltrhr");
...
$group2=implode(",",$group);
   ...
   $checkf=explode(",",",,1,1,1,1,1,,1,,,,1,,,,1");
...
$group2=implode(",",$group);
                $qr=strtolower("select $fields committente as primid FROM ".$rq[1]." group by $group2");
}
  else{
        $qr=strtolower($rq[0]." ,committente as primid FROM ".$rq[1]);}
            }
            if(""!="") $lim="Limit 0,";
        else $lim="";
...
global $adb,$current_language,$current_user;
		...
		$smarty->template_dir = $this->apppath;
                                  ...
                                  $fld=explode(",","id,data,meseanno,commessa,committente,numerochiamate,check21,medialtp,checkltp,adjcheckltp,medialtr,checkltr,adjcheckltr,medialtrhr,checkltrhr,adjcheckltrhr,vis");
                                  $fldn=explode(",","id,data,Month Year,Commessa,CAT,Number of Calls,N.over 21,medialtp,N.over LTP,adjcheckltp,medialtr,checkltr,N.over LTR,medialtrhr,checkltrhr,adjcheckltrhr,Visualizza");
                                          $fldnex=explode(","," ,id,data,Mese-anno,commessa,committente,numero chiamate,check21,medialtp,checkltp,adjcheckltp,medialtr,checkltr,adjcheckltr,medialtrhr,checkltrhr,adjcheckltrhr,Visualizza");
                                        $colex=explode(","," ,Mese-anno,commessa,committente,numero chiamate,check21,checkltp,adjcheckltr ,Visualizza");
                                 ...
                                 $suba=explode(",",",,,,,count,,,,,,,,,,,");
                                        ...
                                        $wdth=explode(",","200,200,120,80,100,60,60,60,60,60,60,60,60,,200,200,150");
                                     $f=Array();
                                     $s=Array();
                                     $a=Array();
                                    $g=Array();
 ...
 $l=0;
 $m=0;
 $fp = fopen('KPI_for_CAT_custom0fd4a0828ac7430cbff7cae2d1923f31.csv', 'w');
     fprintf($fp, chr(0xEF).chr(0xBB).chr(0xBF));
    fputcsv($fp, $colex,";");
    fclose($fp);
  echo "<input type=\"button\" value=\"Export csv\" onclick=\"window.location.href='index.php?module=evvtApps&action=exportcsvKPI_for_CAT_custom0fd4a0828ac7430cbff7cae2d1923f31'\">";
...
$smarty->assign("cond1",$condit);
                	return $smarty->display("ListViewKUIKPI_for_CAT_custom0fd4a0828ac7430cbff7cae2d1923f31.tpl");
        }
