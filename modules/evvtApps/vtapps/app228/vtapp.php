<?php
    ...
    class KPI_for_CAT_custom36ce0a96671423f8cbddb61b859016b1 extends vtApp {
...
$group2=implode(",",$group);
        ...
        $query=$qr."     ".$co." ".$lim;
global $log;
$log->debug('futkot1'); $log->debug($query);
        return $query;
}
...
global $adb,$current_language,$current_user;
		...
		$smarty->template_dir = $this->apppath;
                                  ...
                                  $fld=explode(",",",vis");
                                  $fldn=explode(",",",Visualizza");
                                          $fldnex=explode(","," ,,Visualizza");
                                        $colex=explode(","," , ,Visualizza");
 ...
 $l=0;
 $m=0;
 $fp = fopen('KPI_for_CAT_custom36ce0a96671423f8cbddb61b859016b1.csv', 'w');
     fprintf($fp, chr(0xEF).chr(0xBB).chr(0xBF));
    fputcsv($fp, $colex,";");
    fclose($fp);
  echo "<input type=\"button\" value=\"Export csv\" onclick=\"window.location.href='index.php?module=evvtApps&action=exportcsvKPI_for_CAT_custom36ce0a96671423f8cbddb61b859016b1'\">";
...
$smarty->assign("cond1",$condit);
                	return $smarty->display("ListViewKUIKPI_for_CAT_custom36ce0a96671423f8cbddb61b859016b1.tpl");
        }
