<?php
    ...
    class Dayson_overdue_ID_customf45588abac86d6adc83f3f524032da31 extends vtApp {
...
$fl=implode(",",$fl);
                $rq=explode("from","select $fl from None ");
...
$group2=implode(",",$group);
                $qr=strtolower("select $fields soglieid as primid FROM ".$rq[1]." group by $group2");
}
  else{
        $qr=strtolower($rq[0]." ,soglieid as primid FROM ".$rq[1]);}
            }
            if(""!="") $lim="Limit 0,";
        else $lim="";
...
global $adb,$current_language,$current_user;
		...
		$smarty->template_dir = $this->apppath;
                                  ...
                                  $fld=explode(",","CAT,SOGLIE,soglieid,ValidCall,Repairing,WaitingforParts,RepairCompleted,Closed,Accept/PaymentofQuote,PendingCustomer/Vendor,vis");
                                  $fldn=explode(",","CAT,SOGLIE,soglieid,Valid Call,Repairing,WaitingParts,Repair,Closed,Accept Pay,Pending Cust,Visualizza");
                                          $fldnex=explode(","," ,CAT,SOGLIE,soglieid,Valid Call,Repairing,WaitingParts,Repair,Closed,Accept Pay,Pending Cust,Visualizza");
                                        $colex=explode(","," ,CAT,SOGLIE,Valid Call,Repairing,WaitingParts,Repair,Closed,Accept Pay,Pending Cust ,Visualizza");
                                 ...
                                 $suba=explode(",",",,,,,,,,,,");
                                        ...
                                        $wdth=explode(",","200,200,200,80,80,80,80,80,80,80,150");
                                     $f=Array();
                                     $s=Array();
                                     $a=Array();
                                    $g=Array();
 ...
 $l=0;
 $m=0;
 $fp = fopen('Dayson_overdue_ID_customf45588abac86d6adc83f3f524032da31.csv', 'w');
    fputcsv($fp, $colex,";");
    fclose($fp);
  echo "<input type=\"button\" value=\"Export csv\" onclick=\"window.location.href='index.php?module=evvtApps&action=exportcsvDayson_overdue_ID_customf45588abac86d6adc83f3f524032da31'\">";
...
$smarty->assign("cond1",$condit);
                	return $smarty->display("ListViewKUIDayson_overdue_ID_customf45588abac86d6adc83f3f524032da31.tpl");
        }
