<?php
    ...
    $tit="(Status,Categoria per Numero HelpDesk_Title)";
...
$reportquery=$focus1->sGetSQLforReport($reportid,$nu);
   ...
   $query=$quer[0]."  AND vtiger_troubletickets.status NOT LIKE '%losed%' ";
$i1=0;
...
$dt=Array();
		$rspots=$adb->query("SELECT REPLACE(CONCAT(vtiger_troubletickets.status,' ',vtiger_troubletickets.reportingcategory),\"'\",\"\") as cols FROM $query GROUP BY vtiger_troubletickets.status,vtiger_troubletickets.reportingcategory");
		$data=Array();
                $j=0;
        ...
        $ptc=$pt['cols'];
                       ...
                       $content[$j]["vtigertroubleticketsstatusvtigertroubleticketsreportingcategory"]=$ptc;
                      
        for($m=0;$m<sizeof($cat);$m++){
       $rspots1=$adb->query("SELECT  count(vtiger_troubletickets.title)  as vtigertroubleticketstitle  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.status,' ',vtiger_troubletickets.reportingcategory),\"'\",\"\")='$ptc' and =\"$cat[$m]\" ");
...
$ag[$ii]=$fagg1[$ii].": ".$adb->query_result($rspots1,0,"$mf");
       ...
       $content[$j]["$xf$cat1"]=implode(" - ",$ag);
			...
			$j++;
                ...
                $content[$j]["ticketid"]=$j;
          $content[$j]["vtigertroubleticketsstatusvtigertroubleticketsreportingcategory"]="Totale";
                      
        for($m=0;$m<sizeof($cat);$m++){
       $rspots1=$adb->query("SELECT  count(vtiger_troubletickets.title)  as vtigertroubleticketstitle  FROM $query  and =\"$cat[$m]\" ");
       $mainfld1=explode(",","vtiger_troubletickets.title"); 
       $fagg1=explode(",","Numero HelpDesk_Title");
               for($ii=0;$ii<sizeof($mainfld1);$ii++){
$mf=str_replace(array(".",",","_"),array("","",""),strtolower($mainfld1[$ii]));
