...
global $adb;
$fp = fopen('Astra_report_custom0a177df53cc84b298bea65fb64d7b0fe.csv', 'a');
...
$reportquery=$focus1->sGetSQLforReport($reportid,$nu);
 ...
 $group=array();
  $k=0;
  $fld=explode(",","Project_Progetto_collegato,Project_RMA,Project_ID_progetto,Project_Project_No,Project_Start_Date,Project_Substatus,Project_Stato_parte_chiamata,Project_Days_on,HelpDesk_Ticket_No,HelpDesk_Title,HelpDesk_Status");
...
$group2=implode(",",$group);
        ...
        $qr=strtolower($rq[0]." ,$fname as primid FROM ".$rq[1]);}
            ...
            if(""!="") $lim="Limit 0,";
              ...
              if($_REQUEST["condit"]!="") $co=" and (vtiger_troubletickets.status<>'Closed' or  vtiger_troubletickets.status is null)";
else $co="";
        $query=$adb->query($qr."  ".$co." ".$lim);
         //echo $query; 
        $count=$adb->num_rows($query);
$fld=explode(",","Project_Progetto_collegato,Project_RMA,Project_ID_progetto,Project_Project_No,Project_Start_Date,Project_Substatus,Project_Stato_parte_chiamata,Project_Days_on,HelpDesk_Ticket_No,HelpDesk_Title,HelpDesk_Status,Project_LBLGROUPBUYER");
    $ch=explode(",","");
    if($count!=0){
for($i=0;$i<$count;$i++){
