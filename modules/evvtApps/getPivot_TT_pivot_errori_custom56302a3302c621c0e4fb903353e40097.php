<?php
    ...
    $tit="(WEEK,Sorgente per Valore min Title)";
...
$dt=Array();
		$rspots=$adb->query("SELECT REPLACE(CONCAT(vtiger_troubletickets.fiscalwint,' ',vtiger_troubletickets.sorgentett),\"'\",\"\") as cols FROM $query GROUP BY vtiger_troubletickets.fiscalwint,vtiger_troubletickets.sorgentett");
		$data=Array();
                $j=0;
        ...
        $ptc=$pt['cols'];
                       ...
                       $content[$j]["vtigertroubleticketsfiscalwintvtigertroubleticketssorgentett"]=$ptc;
              unset($ag1);        
        for($m=0;$m<sizeof($cat);$m++){
       $rspots1=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid,min(vtiger_troubletickets.title)  as vtigertroubleticketstitle  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.fiscalwint,' ',vtiger_troubletickets.sorgentett),\"'\",\"\")='$ptc' and vtiger_troubletickets.status=\"$cat[$m]\" ");
       $mainfld1=explode(",","vtiger_crmentity.crmid,vtiger_troubletickets.title"); 
       $fagg1=explode(",","Numero ID,Valore min Title");
           if(""=="1"){
            $dttod=date("Y-m-d",strtotime("-1 years",strtotime(date("Y-m-d"))));
             $dttod2=date("Y-m-d",strtotime("-2 years",strtotime(date("Y-m-d"))));
       $rspots22=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid,min(vtiger_troubletickets.title)  as vtigertroubleticketstitle  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.fiscalwint,' ',vtiger_troubletickets.sorgentett),\"'\",\"\")='$ptc' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')>'$dttod2' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')<='$dttod' and vtiger_troubletickets.status=\"$cat[$m]\"");
...
$ag2[$ii]+=$adb->query_result($rspots1,0,"$mf");
     ...
     if(""=="1"){
            $dttod=date("Y-m-d",strtotime("-1 years",strtotime(date("Y-m-d"))));
             $dttod2=date("Y-m-d",strtotime("-2 years",strtotime(date("Y-m-d"))));
       $rspots2=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid,min(vtiger_troubletickets.title)  as vtigertroubleticketstitle  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.fiscalwint,' ',vtiger_troubletickets.sorgentett),\"'\",\"\")='$ptc' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')>'$dttod2' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')<='$dttod'");
...
$agy2[$ii1]+=$adb->query_result($rspots2,0,"$mf");
  ...
  $rspots3=$adb->query("SELECT  sum($f[$ik]) as form  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.fiscalwint,' ',vtiger_troubletickets.sorgentett),\"'\",\"\")='$ptc' ");
  ...
  if(""!=""){
      ...
      $fcols[$ik]+=$f1[$ik];
			...
			$j++;
                ...
                $content[$j]["ticketid"]=$j;
          $content[$j]["vtigertroubleticketsfiscalwintvtigertroubleticketssorgentett"]="Totale";
         $content[$j]["totale"]=implode(" - ",$ag2);  
        for($m=0;$m<sizeof($cat);$m++){
       $rspots1=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid,min(vtiger_troubletickets.title)  as vtigertroubleticketstitle  FROM $query  and vtiger_troubletickets.status=\"$cat[$m]\" ");
       $mainfld1=explode(",","vtiger_crmentity.crmid,vtiger_troubletickets.title"); 
       $fagg1=explode(",","Numero ID,Valore min Title");
           $cat1=str_replace(array(".",",","_"," "),array("","","",""),strtolower($cat[$m]));
          $xf=str_replace(array(".",",","_"," "),array("","","",""),strtolower("Status"));
