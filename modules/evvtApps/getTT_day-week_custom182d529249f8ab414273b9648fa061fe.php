<?php
    ...
    $tit="(Giorno apertura per Numero HelpDesk_ID)";
...
$dt=Array();
       ...
       $rspots1=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.opendate),\"'\",\"\")='$ptc' and vtiger_troubletickets.reportingcategory=\"$cat[$m]\" ");
...
$ag2[$ii]+=$adb->query_result($rspots1,0,"$mf");
     ...
     if(""=="1"){
       ...
       $rspots2=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid  FROM $query  and  REPLACE(CONCAT(vtiger_troubletickets.opendate),\"'\",\"\")='$ptc' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')>'$dttod2' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')<='$dttod'");
