...
global $adb;
$fp = fopen('testtttttttttttttttt_custom14c69433d7e8b177aa95af7c0fcf44a3.csv', 'a');
...
$checkf=explode(",","1,1,1,1,1,1,1,1");
 ...
 $group=array();
  $k=0;
  $fld=explode(",","Project_Progetto_collegato,Project_Project_Name,Project_Project_No,Project_Substatus,Project_RMA,Project_Assigned_To,Project_ID_progetto");
...
for($i=0;$i<$count;$i++){
   $content[$i]["projectid"]=$adb->query_result($query,$i,"primid");
       $content2[$i]["projectid"]=$adb->query_result($query,$i,"primid");
...
$us=getUsername($adb->query_result($query,$i,"$name"));
   ...
   $content[$i]["$name1"]=$us; $content2[$i]["$name1"]=$us;}
         ...
         if($adb->query_result($query,$i,"$name")==null) $s="";
          ...
          $content2[$i]["$name1"]=number_format($s,0,",",".");
