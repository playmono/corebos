...
global $adb;
$fp = fopen('_custom73c7ff097716e75c531eb75cc10f646c.csv', 'a');
...
$checkf=explode(",","1,1,1,1,1,1,1");
 ...
 $group=array();
  $k=0;
  $fld=explode(",","Accounts_Account_Name,Accounts_Natura_Giuridica,Accounts_Type,Accounts_industry,Accounts_Partita_Iva,Accounts_Is_vendor");
...
$group2=implode(",",$group);
                $qr=strtolower("select $fields vtiger_contactdetails.contactid as primid FROM ".$rq[1]." group by $group2");
}
        else{
        $qr=strtolower($rq[0]." ,vtiger_contactdetails.contactid as primid FROM ".$rq[1]);}
            }
...
require('user_privileges/user_privileges_'.$current_user->id.'.php');
 if($is_admin==false && $profileGlobalPermission[2] == 1 && ($defaultOrgSharingPermission[getTabid("")] == 3 or $defaultOrgSharingPermission[getTabid("")] == 0))
        ...
        $query=$adb->query($qr."  and (fieldname1='test' or fieldname2 LIKE '%test%') ".$co." ".$condd." ".$lim);
...
$fld=explode(",","Accounts_Account_Name,Accounts_Natura_Giuridica,Accounts_Type,Accounts_industry,Accounts_Partita_Iva,Accounts_Is_vendor");
    ...
    if($count!=0){
for($i=0;$i<$count;$i++){
   $content[$i]["contactid"]=$adb->query_result($query,$i,"primid");
...
$us=getUsername($adb->query_result($query,$i,"$name"));
   ...
   $content[$i]["$name1"]=$us;}
       ...
       $content[$i]["url"]="index.php?module=$md1&action=DetailView&record=". $content[$i]["contactid"];
