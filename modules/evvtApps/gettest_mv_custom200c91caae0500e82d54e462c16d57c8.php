...
$reportquery=$focus1->sGetSQLforReport($reportid,$nu);
 ...
 $group=array();
  $k=0;
  $fld=explode(",","pname_template,projectid,progetto,projectname,projectstatus,substatusproj,projecttaskid,pt_substatus,status_pt,projecttaskname,timecontrolid,substatus,case_tc,title,status_timecontrol,date_start,time_start,date_end,time_end,deleted");
...
$group2=implode(",",$group);
        ...
        $qr=strtolower($rq[0]." ,.id as primid FROM ".$rq[1]);}
              ...
              $query1="show columns from mv_testtc";
...
$j=0;
	...
	for($i1=1;$i1<=$num_rows1;$i1++)
	{
       ...
       $md1=$adb->query_result($tab,0,"name");
             ...
             $fl=implode(",",$fl);
                $rq=explode("from","select $fl from mv_testtc ");
