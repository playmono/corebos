<?php
    ...
    $tit="(CAT TPV per Numero N. chiamate)";
...
$frm[$ik]+=$adb->query_result($rspots3,0,"form");}
  ...
  if(""!=""){
      ...
      $fcols[$ik]+=$f1[$ik];
        ...
        for($m=0;$m<sizeof($cat);$m++){
       $rspots1=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid  FROM $query  and vtiger_project.raggruppamento_giorni=\"$cat[$m]\" ");
       $mainfld1=explode(",","vtiger_crmentity.crmid"); 
       $fagg1=explode(",","Numero N. chiamate");
           $cat1=str_replace(array(".",",","_"," "),array("","","",""),strtolower($cat[$m]));
          $xf=str_replace(array(".",",","_"," "),array("","","",""),strtolower("Giorni"));
