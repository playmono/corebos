<?php
    ...
    $tit="(Creazione per Numero HelpDesk_ID)";
...
$dt=Array();
		$rspots=$adb->query("SELECT REPLACE(CONCAT(vtiger_crmentityRelHelpDesk.createdtime),\"'\",\"\") as cols FROM $query GROUP BY vtiger_crmentityRelHelpDesk.createdtime");
		$data=Array();
                $j=0;
        ...
        $ptc=$pt['cols'];
                       ...
                       $content[$j]["vtigercrmentityhelpdeskcreatedtime"]=$ptc;
              unset($ag1);        
        for($m=0;$m<sizeof($cat);$m++){
       $rspots1=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid  FROM $query  and  REPLACE(CONCAT(vtiger_crmentityRelHelpDesk.createdtime),\"'\",\"\")='$ptc' and vtiger_troubletickets.status=\"$cat[$m]\" ");
...
$ag2[$ii]+=$adb->query_result($rspots1,0,"$mf");
     ...
     if(""=="1"){
       ...
       $rspots2=$adb->query("SELECT  count(vtiger_crmentity.crmid)  as vtigercrmentitycrmid  FROM $query  and  REPLACE(CONCAT(vtiger_crmentityHelpDesk.createdtime),\"'\",\"\")='$ptc' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')>'$dttod2' and DATE_FORMAT(vtiger_crmentityHelpDesk.createdtime,'%Y-%m-%d')<='$dttod'");
...
$agy2[$ii1]+=$adb->query_result($rspots2,0,"$mf");
  ...
  if(""!=""){
  $rspots3=$adb->query("SELECT  sum() as form  FROM $query  and  REPLACE(CONCAT(vtiger_crmentityHelpDesk.createdtime),\"'\",\"\")='$ptc' ");
...
$frm+=$adb->query_result($rspots3,0,"form");
			...
			$j++;
                ...
                $content[$j]["ticketid"]=$j;
          $content[$j]["vtigercrmentityhelpdeskcreatedtime"]="Totale";
